...
This headline result is regenerated by `python all_encounters_group_split_search.py`, not by the
first-encounter training section at the start of `hospital_readmission_modeling.ipynb`. The
modeling notebook displays this result in its Extended Experiment Search section from the saved
CSV audit trail after the script has been run.
